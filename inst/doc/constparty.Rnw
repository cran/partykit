...
%\VignetteDepends{partykit, rpart, RWeka, datasets}
...
from package \pkg{rpart} \citep{pkg:rpart} to this data set (make sure to set
...
The \pkg{RWeka} package \citep{pkg:RWeka} provides an interface to the
...
%%% <NOTE>pmml was archived Jan 2026</NOTE>
...
%\begin{figure}[t!]
%\centering
%<<PMML-Titanic-plot1, fig.height = 9, fig.width = 17>>=
%plot(ttnc_quest)
%@
...
%\begin{figure}[t!]
%\centering
%<<PMML-Titanic-plot2, fig.height = 8.5, fig.width = 14>>=
%plot(ttnc_quest2)
%@
...
%package \pkg{pmml} \citep{pkg:pmml}, part of the \pkg{rattle} project
%\citep{rattle}, that allows to
%export PMML files for \pkg{rpart} trees from \proglang{R}. For example,
%with \pkg{pmml} available, for the \class{rpart} tree for the Titanic data:
%<<PMML-write, message = FALSE>>=
%if (pmmlOK <- require("pmml")) {
%    tfile <- tempfile()
...
%    print(all.equal(predict(party_rp, newdata = ttnc, type = "prob"), 
...
%Group web page, e.g.,
%\url{https://dmg.org/pmml_examples/rattle_pmml_examples/AuditTree.xml} or
%\url{https://dmg.org/pmml_examples/rattle_pmml_examples/IrisTree.xml}.
%
